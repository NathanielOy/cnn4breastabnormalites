...
import os
import pandas as pd
from latest_input_fun4 import input_fn_test, input_fn_train, validation_fn_inputs, train_fn_inputs, get_training_data_old,  \
...
"""
from augmentation_op import aug_train, aug_validation, aug_test, aug_train_mias3, aug_train_mias2, aug_train_mias1, aug_train_ddsm1, aug_train_ddsm2
...
if __name__ == "__main__":
    ...
    dataWant='ddsm' #numpy_mias, inbreast, numpy_only, large_mias
    ...
    if dWhat==-1:
        ...
        train_dataset=numpy_train_data(K.get_session(), batch_size)#, train_fn_inputs(K.get_session(), epochs, batch_size, trainaug) #
        val_data=numpy_validation_data(K.get_session(), batch_size)#, validation_fn_inputs(K.get_session(), epochs, batch_size, valaug) #
        test_dataset =numpy_test_data(K.get_session(), batch_size)#, test_fn_inputs(K.get_session(), epochs, batch_size, None) test_fn_inputs(K.get_session(), epochs, batch_size, testaug) #
    """
    ...
    mydir='/mdata/'  #'./'    #'./'  '/content/gdrive/My Drive/'
    print(hist.history)
