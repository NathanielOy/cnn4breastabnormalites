# -*- coding: utf-8 -*-
"""
Created on Sat Oct 26 09:36:15 2019
...
def get_batches(X, y, batch_size, filenames=None, distort=False, shuffle=True):
    shuffled_idx = np.arange(int(y.shape[0])) #y.shape[0]   len(y)  tf.size(y)  tf.shape(y)[0]  len(y.get_shape()) tf.shape(y)[0]
    ...
    for i in range(0, int(y.shape[0]), batch_size):
